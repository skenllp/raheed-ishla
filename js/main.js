/* ==========================================================================
   AL RAHEED ARAFATH & ZUBAIDA SANA SHERIF — WEDDING INVITATION
...
(() => {
  ...
  function renderContent() {
    ...
    setText("[data-bind='hero-bride-name']", cfg.couple.heroBrideName || cfg.couple.brideFirstName);
    ...
    // Family/relationship line is optional — hide it gracefully rather than
    // rendering blank or partial text when no family details are supplied.
    setOptionalDetails("bride", cfg.couple.bride, "D/o");
    ...
    setOptionalDetails("groom", cfg.couple.groom, "S/o");

    // Wedding details
    setText("[data-bind='detail-date']", cfg.wedding.dateDisplay);
    setText("[data-bind='detail-day']", cfg.wedding.dayDisplay);
    // Hijri date is optional — hide the line entirely when not supplied,
    // rather than leaving an empty gap in the card.
    const hijriEl = document.querySelector("[data-bind='detail-hijri']");
    if (hijriEl) {
      if (cfg.wedding.hijriDate) {
        hijriEl.textContent = cfg.wedding.hijriDate;
    ...
    // Wishes / Sponsor
    if (cfg.wishes) {
      setText("[data-bind='wishes-kicker']", cfg.wishes.kicker);
      setText("[data-bind='wishes-heading']", cfg.wishes.heading);
      setText("[data-bind='wishes-sponsor-name']", cfg.wishes.sponsorName);
      const wishesLogo = document.querySelector("[data-bind='wishes-logo']");
      if (wishesLogo && cfg.wishes.sponsorLogo) {
        wishesLogo.src = cfg.wishes.sponsorLogo;
        wishesLogo.alt = cfg.wishes.sponsorName || "Sponsor logo";
      }
    }
  ...
  // Fills a couple card's address/parents line, or hides the whole line
  // gracefully when no family details were supplied in the config.
  function setOptionalDetails(who, person, relation) {
    const addressEl = document.querySelector(`[data-bind='${who}-address']`);
    const parentsEl = document.querySelector(`[data-bind='${who}-parents']`);
    const wrapEl = document.querySelector(`[data-bind='${who}-details']`);
    const hasAddress = !!(person.house || person.po);
    const hasParents = !!(person.father || person.mother);
    if (addressEl) addressEl.textContent = hasAddress ? `${person.house}, ${person.po}`.replace(/^, |, $/g, "") : "";
    if (parentsEl) parentsEl.textContent = hasParents ? `${relation} ${person.father}${person.father && person.mother ? " & " : ""}${person.mother}` : "";
    if (wrapEl) wrapEl.style.display = (hasAddress || hasParents) ? "" : "none";
  }
